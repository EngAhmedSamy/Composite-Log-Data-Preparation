...
# Load default icons from repo (assume icons/ folder with 17.5.png, etc.)
common_sizes = ["8.5\"", "12.25\"", "17.5\""]
for size in common_sizes:
    file_name = size.replace('"', '') + '.png'
    try:
        with open(f"icons/{file_name}", "rb") as f:
            st.session_state.icons[size] = f.read()
    except FileNotFoundError:
        pass  # Will warn later if missing

# Section: Optional override icons upload
with st.expander("Override Bit Icons (optional)"):
    for size in common_sizes:
        uploaded_icon = st.file_uploader(f"Upload/Override icon for {size} (PNG/JPG)", type=["png", "jpg", "jpeg"], key=f"icon_{size}")
        if uploaded_icon:
            st.session_state.icons[size] = uploaded_icon.getvalue()
            st.success(f"Icon for {size} updated")
