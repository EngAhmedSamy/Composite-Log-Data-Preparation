            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if depth_v and text_parts:
                        try:
                            d = int(float(depth_v))
                            quoted_text = build_quoted_text(text_parts, label_width=10, value_width=20)  # ← key line
            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if depth_v and text_parts:
                        try:
                            d = int(float(depth_v))
                            quoted_text = build_quoted_text(text_parts, label_width=10, value_width=12)  # ← same width
                            data.append((d, quoted_text))
                        except:
                            continue
